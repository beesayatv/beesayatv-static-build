(function(window, L) {
    ...
    function create(map, options) {
        ...
        function loadData() {
            ...
            loadingPromise = fetch(options.manifestUrl, { cache: 'no-store' })
        ...
        function render() {
            ...
            // Every public observation stays at its recorded GeoJSON coordinate.
            // The former zoom-grid thinning replaced one observation with another
            // as zoom changed, which looked like pulses drifting away from routes.
            // This public dataset is deliberately small enough for Leaflet markers.
            var zoom = map.getZoom();
            var visible = measurements.slice().sort(stableFeatureOrder);
